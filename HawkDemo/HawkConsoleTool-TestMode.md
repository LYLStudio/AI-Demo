### 變更說明（重點）
我已將 `HawkConsoleTool.java` 修改為**單一檔案**且新增 **「測試模式（--test）」**，以便在不依賴真實 TIBCO Hawk 環境下進行整合測試。主要變更如下：

- **新增 `ManagerInterface`**：抽象出 `FullManager` 的外部 API（REST server 使用的那些方法），讓 REST server 可以接受任意實作（真實 `FullManager` 或 `TestManager`）。
- **`FullManager` 改為實作 `ManagerInterface`**（原本功能不變）。
- **新增 `TestManager`（測試模式）**：模擬 agent/microagent、支援可配置的「長時間執行方法」以測試 invocation timeout、Semaphore 限流、metrics、refresh token 持久化等行為。`TestManager` 不依賴 TIBCO Hawk API，適合 CI / 本機測試。
- **`JwtAuthRestServer` 現在接受 `ManagerInterface`**，因此可以在真實或測試模式下運作而不需修改 REST 程式碼。
- **啟動參數**：加入 `--test` 開關（例如 `java -cp ... HawkConsoleTool --test 8080`）以啟動測試模式。若不使用 `--test`，程式會嘗試建立真實 `TIBHawkConsole`（原行為）。

下方提供完整單一檔案 `HawkConsoleTool.java`（含測試模式），以及如何使用測試模式與測試範例說明。
...
1. **編譯**
   ```bash
   javac -cp "path/to/gson.jar:." HawkConsoleTool.java
   ```
   - 若要使用真實 Hawk 功能（非測試模式），請在 classpath 加上 Hawk 與 EMS 的 jar（例如 `hawk-console-api.jar`、`tibjms.jar` 等）。

2. **啟動（測試模式）**
   ```bash
   java -cp "path/to/gson.jar:." HawkConsoleTool --test 8080
   ```
   - 這會啟動 REST server（預設 port 8080）與 `TestManager`。`TestManager` 模擬 3 個 agent，每個 agent 有 microagent（包含 `longSleep` 方法）。

3. **測試 invocation timeout 與限流**
   - 呼叫 `/control` 來觸發 `longSleep`（會睡 15 秒，超過預設 `INVOCATION_TIMEOUT_MS` 10 秒，會被取消並回報 timeout）：
     ```bash
     curl -s -X POST "http://localhost:8080/control" -H "Content-Type: application/json" -d '{"action":"longSleep","micro":"bw-1","agent":"agent-1"}' | jq
     ```
   - 呼叫多次以測試 Semaphore 限流（同時多個 longSleep 會被限制）。

4. **檢查 metrics**
   ```bash
   curl -s http://localhost:8080/metrics | jq
   ```
   - 你會看到 `invocationTimeouts`、`invokedMethods`、`invocationFailures` 等指標。

5. **測試 refresh token persistence**
   - 登入：
     ```bash
     curl -s -X POST http://localhost:8080/login -H "Content-Type: application/json" -d '{"username":"demo-user","password":"demo-password"}' | jq
     ```
   - 檢查 `refresh_tokens.json` 是否包含 refresh token（檔案會在當前工作目錄產生）。

6. **切回真實模式**
   - 若要連接真實 Hawk，啟動時不要加 `--test`，並提供 PEM 金鑰路徑（或允許產生 ephemeral key for demo）：
     ```bash
     java -cp "path/to/hawk-console-api.jar:path/to/tibjms.jar:path/to/gson.jar:." HawkConsoleTool 8080 /path/to/priv.pem /path/to/pub.pem
     ```
...
## 為何這樣設計（簡短）
- **不改變 REST API**：測試模式與真實模式共用相同 REST 介面，方便 CI 與整合測試。
- **單一檔案**：維持使用者要求的單一檔案結構，方便快速部署與檢視。
- **可測試性**：`TestManager` 提供可預測的長時間方法（`longSleep`），可用來驗證 timeout、取消、限流與 metrics。
- **最小侵入**：真實 `FullManager` 保留（以 reflection 嘗試與 Hawk 互動），但測試模式不依賴 Hawk。

---

如果你要我接著做：
- **把 `TestManager` 的 longSleep 時間、並發數、或模擬錯誤行為暴露為 REST 可配置參數**（方便 CI 動態調整）。
- **把 `JwtUtil` 換成 Nimbus JOSE + JWT 的實作（含驗證）**。
- **把 `TestManager` 改為可注入的 mock（JUnit-friendly）並產生 JUnit 測試範例**。
